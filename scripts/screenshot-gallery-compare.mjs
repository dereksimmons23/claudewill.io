...
async function run() {
  ...
  for (const option of ['marquee', 'grid']) {
    for (const vp of [
      ...
      // Toggle gallery option
      if (option === 'grid') {
        await page.evaluate(() => {
          document.getElementById('gallery-marquee').style.display = 'none';
      ...
      const filename = `gallery-${option}-${vp.name}.png`;
      await page.screenshot({ path: join(screenshotDir, filename), fullPage: false });
      const kb = (statSync(join(screenshotDir, filename)).size / 1024).toFixed(1);
  ...
  await browser.close();
  server.close();
  console.log('Gallery comparison screenshots saved.');
}
